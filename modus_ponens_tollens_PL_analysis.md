...
Every claim below is tiered. Claims tagged **[FORCED — block *n*]** were decided by complete enumeration or exact computation in the companion script `mp_mt_verify.py`, whose numbered blocks this document cites; **[EMPIRICAL]** claims were measured with stated bounds; **[PRESUMED — source]** claims are inherited from the literature and priced at zero here; **[STIPULATED]** marks conventions this analysis declares. The mechanics come first, the reification and truth-presumption analysis second, because the second is only visible once the first is on the table.
...
The probability carrier prices the two rules asymmetrically, and the asymmetry is arithmetic, not philosophy. The MP-direction bound P(B) ≥ P(A) + P(A→B) − 1 held on all fifty random probability spaces tried [FORCED — block 6] — support propagates, bounded and gradual. But run the instance P(A)=0.5, P(B|A)=1, P(B|¬A)=0.5: observing B (the MP direction) lifts A only to 0.667, while observing ¬B (the MT direction) drops A to exactly 0.000 — **total refutation from a single datum**, because a degree-1 conditional makes ¬B and A jointly impossible [FORCED — block 6]. Confirmation accrues; refutation detonates. Popper's asymmetry falls out of Bayes with no epistemology added [PRESUMED as the philosophical framing — Popper 1934; the arithmetic is FORCED above].
...
**Discoverable → refutation-totality.** If every proposition has its value out there awaiting discovery, then a prediction that failed to verify has *thereby* been discovered false — every non-verification is a counterexample, and MT's trigger arms on absence. Withdraw discoverability and the gap opens: K3's policy, where b=½ arms nothing [FORCED — block 5], is exactly the policy that verification-transcendent truth forbids. The choice between "unverified" and "refuted" as readings of a failed prediction is the choice between constructed and discovered designation, made operational. Nothing in the MT schema decides it; θ decides it.

Now the other side of the ledger — **what survives the presumption's removal** — and this is the sharpest result available here. MP survives *entirely*. Under BHK it is function application; under residuation it is definitional; block 2's identity T(a, a→b) = min(a,b) needs no bivalence, no realism, no discovery. MT survives as composition toward ⊥ [FORCED — block 4]. What is lost is exactly and only the *free* versions: free storage, lossless detachment, the mirror, refutation-from-absence. **The presumption of truth as an intrinsic discrete discoverable primitive is not the license for inference. It is the license for inference at zero marginal cost.** The engine runs without the doctrine; the doctrine's whole function is to unplug the meter.
...
**Tier-weighted detachment.** MP composes by weakest link: the conclusion's tier is the minimum over {antecedent tier, certificate tier, the three riders' tiers}. This is `run_claim` plus min-composition, running today. A conclusion is never better than its worst rider, and the worst rider is usually one the classical presentation didn't book.

**Detachment privileges by tier.** FORCED tokens may detach clean — they travel naked, because re-derivation at original price is guaranteed. STIPULATED tokens detach *with the stipulation conserved*: the rider list is attached to every downstream conclusion, which is precisely the "/STIPULATED" suffix that PL-Verify and the carriersets verdicts refuse to drop. Full classical detachment — value only, provenance burned — is a privilege of the mathematical regime, where Rider 3 is enforced by the subject matter.

**Tier-routed MT.** On ¬B, route the refutation to the lowest-tier link of the failed conjunction, not automatically to A. Publish the routing table with the inference. The Living Map's refusal semantics is the production instance: probes refute contract-membership, never the arithmetic.
...
**Leak budgets for chains.** In any graded or empirical regime, an MP chain of length n with certificates at degree 1−ε carries a guarantee of at most 1−nε [FORCED — block 3]. Budget n before building the chain. The discrete regime's exemption from this rule is real but must be *earned* at minting time — the Boundary Law's D = 8σ²ln(1/δ)/gap² per distinction — and a chain is only as certified as its most expensive unpaid distinction.

**Never reify the rule.** Gradients are not values; an inference engine that stores its own rules as ordinary premises stalls in Carroll's regress [FORCED — block 7]. Keep G and V typed apart; the metatheory is not a hiding place for costs but it is the correct home for operations.
...
Complete output of `mp_mt_verify.py` (deterministic, ~1s, no dependencies beyond the standard library):

```
[1] MP/MT designation-validity, complete enumeration
  CL2   material, D={1}               MP HOLDS      MT HOLDS
  K3    material, D={1}               MP HOLDS      MT HOLDS
  LP    material, D={1/2,1}           MP FAILS (1/2, 0)   MT FAILS (1, 1/2)
  L3    Luk IMP,  D={1}               MP HOLDS      MT HOLDS
  L3+   Luk IMP,  D={1/2,1}           MP FAILS (1/2, 0)   MT FAILS (1, 1/2)
  G3    residuum, D={1}               MP HOLDS      MT HOLDS
  WK3   weak material, D={1}          MP HOLDS      MT HOLDS
  POST4 cyclic neg, material, D={1}   MP HOLDS      MT HOLDS (vacuously meaningful-free)
  FOUR  material, D={T,B}             MP FAILS (B, N)     MT FAILS (T, B)

[2] Lukasiewicz: T(a, a->b) = min(a,b) at all 441 grid points: True
    Godel and product residuation inequalities: True, True

[3] leak: n=1 -> 0.999   n=100 -> 0.900   n=500 -> 0.500   n=1000 -> 0.000

[4] 3-chain Heyting: MT valid (min = 1 over all 9 points);
    mirror (~b->~a)->(a->b) = 1/2 at (a,b)=(1, 1/2)

[5] K3 trigger: b=1/2 -> NEG(b)=1/2, not armed; b=0 -> NEG(b)=1, armed

[6] P(B) >= P(A)+P(A->B)-1 on 50/50 random spaces;
    instance: P(A|B)=0.667 (bounded lift), P(A|~B)=0.000 (total refutation)

[7] tortoise: 5 rounds, 7 premises, B never detaches
```

## Appendix B · Cross-references into the MathofLogic stack

The kernel-parity claims: LP's MP failure matches `pl.py`'s 15-law signature and carriersets' `lp_signature`; K3's clean detachment matches its 14/15 lone-LEM signature (`k3_signature`, both readings decided). The BHK step-counting: `lambda_church_rosser_bounded`. The distinction pricing: `boundary_law_scaling`. The tier-routing in production: `program_variant_contract_gate` and the PL-Verify verdict cap. The relational truth machinery: `tarski_satisfaction_finite`. Reproduce everything: `python3 mp_mt_verify.py`.
